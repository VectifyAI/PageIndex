...
def parse_charlevel_meta(doc_handle: Union[str, Path, BytesIO]) -> tuple[list[list[Span]], list]:
    ...
    for page_idx in range(len(pdf)):
        ...
        if pdf_doc is not None and page_idx < pdf_doc.page_count:
            try:
                ...
                show_flush_ids, show_fonts, show_text_units, show_tzs, xobject_paints = _tokenize_show_operators(
                    pdf_doc[page_idx].read_contents())
            except Exception:
                show_fonts = []
                show_tzs = []
            vert_names = _page_vertical_resource_names(pdf_doc, page_idx)
            # Patch per-char unicode to span merger glyph Unicode where PDFium's
            # decode differs (guarded: any failure keeps PDFium's output).
            if raw_chars:
                try:
    ...
    for page_view_index, raw_chars in enumerate(per_page):
        ...
        merged = _merge_text_items(fin, page_view_boxes[page_view_index])
        ...
        merged = _remerge_oblique(merged, fin)  # oblique objects: inverse-rotation projection re-merge
        raw_pages.append(merged)
